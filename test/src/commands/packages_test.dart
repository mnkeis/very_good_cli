...
const expectedPackagesUsage = [
  ...
  'Command for managing packages.\n'
      ...
      '  upgrade   Upgrade packages in a Dart or Flutter project.\n'
      '\n'
...
const expectedPackagesUpgradeUsage = [
  // ignore: no_adjacent_strings_in_list
  'Upgrade packages in a Dart or Flutter project.\n'
      '\n'
      'Usage: very_good packages upgrade [arguments]\n'
      '-h, --help              Print this usage information.\n'
      '''-r, --recursive         Upgrade dependencies recursively for all nested packages.\n'''
      '    --major-versions    Upgrade dependencies to major versions.\n'
      '\n'
      'Run "very_good help" to see global options.'
];
    ...
    group('upgrade', () {
      test(
        ...
        withRunner((commandRunner, logger, printLogs) async {
          ...
          final resultAbbr =
              await commandRunner.run(['packages', 'upgrade', '-h']);
          expect(printLogs, equals(expectedPackagesUpgradeUsage));
          expect(resultAbbr, equals(ExitCode.success.code));
        }),
      );
      ...
      test(
        ...
        withRunner((commandRunner, logger, printLogs) async {
          ...
          File(path.join(directory.path, 'pubspec.yaml')).writeAsStringSync('');
          final result = await commandRunner.run(
            ['packages', 'upgrade', directory.path],
          );
          expect(result, equals(ExitCode.unavailable.code));
        }),
      );
      ...
      test(
        ...
        withRunner((commandRunner, logger, printLogs) async {
          ...
          final result = await commandRunner.run(
            ['packages', 'upgrade', '-r', tempDirectory.path],
          );
          expect(result, equals(ExitCode.noInput.code));
          verify(() {
            logger.err(any(that: contains('Could not find a pubspec.yaml in')));
